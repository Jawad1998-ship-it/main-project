            ...
            using (var connection = _db.GetConnection())
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    try
                    {
                        ...
                        using (var cmd = new SqlCommand("UPDATE Volunteer SET IsActive = 1, UpdatedAt = GETDATE() WHERE VolunteerID = @VolunteerID", connection, transaction))
                        {
                        ...
                        using (var cmd = new SqlCommand("UPDATE VolunteerAssignment SET Status = 'Accepted' WHERE VolunteerID = @VolunteerID AND ProjectID = @ProjectID", connection, transaction))
                        {
                            cmd.Parameters.AddWithValue("@VolunteerID", volunteerId);
